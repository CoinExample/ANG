...
Pcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month. 
...
mnbudget prepare proposal-name url payment_count block_start pcoin_address monthly_payment_pcoin [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ PCOIN, _12_ times over the course of a year totaling _24000_ PCOIN. 
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` PCOIN. 

The following new RPC commands are supported:
- mnbudget "command"... ( "passphrase" )
 - prepare            - Prepare proposal for network by signing and creating tx
 - submit             - Submit proposal for network
 - vote-many          - Vote on a Pcoin initiative
 - vote-alias         - Vote on a Pcoin initiative
 - vote               - Vote on a Pcoin initiative/budget
