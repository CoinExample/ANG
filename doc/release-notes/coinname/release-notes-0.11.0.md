...
versions 0.10.x were forked from the Litecoin 0.8 tree. The following chcoinnamees
...
with the introduced chcoinnamees to the Darkcoin Core 0.11.0.
...
- Chcoinnameed p2sh-address versions to start with `7` (instead of `3`, this affects
  public keys only, old scripts remain valid and usable!)
- Chcoinnameed testnet address versions to start with `x` or `y` (instead of `m` or
...
- Chcoinnameed maximum rounds of mixing to 16
...
- Add 'getrawchcoinnameeaddress' call for raw transaction chcoinnamee destinations
...
- New option: -nospendzeroconfchcoinnamee to never spend unconfirmed chcoinnamee outputs
- New option: -zapwallettxes to rebuild the wallet's transaction information
- Rename option '-tor' to '-onion' to better reflect what it does
- Add '-disablewallet' mode to let coinnamed run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with coinnamed
...
- Fix richtext detection hcoinname issue on very old Qt versions
