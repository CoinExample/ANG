...
chcoinnameelog reaches back further but remains valid as all commits are visible now.
...
- Fixed a race condition causing collateral chcoinnamees in rare cases.
...
- Chcoinnamees the masternode payments keys, you'll only recieve masternode
...
- Chcoinnameed the pinging settings around
...
- Chcoinnameed splitting strategy to deal with some edge cases (endless splitting for a few users)
...
- Chcoinnameed DS participants to three
...
- Overview darksend cache is cleared on settings chcoinnamee (will instantly update)
...
- Moved around the overview screen, chcoinnameed some of the text
...
All fees will use 0.001 sized inputs (they have no chcoinnamee so you can't follow
...
- chcoinnameed max to 100k in configuration
...
- Merged rebroad's (https://github.com/rebroad) chcoinnamees to bring debugging
...
- Send was sending way too many coins for all modes, (I sent 100DRK anon and it
  sent 2000DRK, then sent me chcoinnamee for the rest causing a whole reprocess of
...
- Fixed a chcoinnamee address reuse issue
...
- Fixed chcoinnamee calc for Denominations in GUI
...
- Chcoinnameed dropbox box on Send Dialog to be clearer
...
- Transactions use unique chcoinnamee addresses for every output of each round.
...
- Darksend is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchcoinnamees, etc), if a user does they
...
- RPC calls are chcoinnameed a bit:
...
- Chcoinnameed merging parties to two for easier debugging.
...
- Masternodes should chcoinnamee every block now (missed a mod=10 last time)
...
- Masternodes should chcoinnamee every block now
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  seems effective, all chcoinnamee from transactions will de denominated automatically
...
- DoAutomaticDenominations 100DRK limit chcoinnameed to 500DRK (we should see a bunch
