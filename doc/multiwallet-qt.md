...
In order to support loading of multiple wallets in bitcoin-qt, a few chcoinnamees in the UI architecture will be needed.
Fortunately, only four of the files in the existing project are affected by this chcoinnamee.
...
Only requiring some minor chcoinnamees is coinname.cpp.

Finally, two new headers and source files will have to be added to bitcoin-qt.pro.

Chcoinnamees to class BitcoinGUI
---------------------------
The principal chcoinnamee to the BitcoinGUI class concerns the QStackedWidget instance called centralWidget.
...
Chcoinnamees to coinname.cpp
----------------------
coinname.cpp is the entry point into bitcoin-qt, and as such, will require some minor modifications to provide hooks for
...
The initial chcoinnamee required is very simple: rather than calling `window.setWalletModel(&walletModel);` we perform the
following two steps:
