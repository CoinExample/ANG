...
*Setup instructions for a gitian build of Pcoin using a Debian VM or physical system.*
...
Clone the git repositories for pcoin and gitian and then checkout the pcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pcoin repository
...
To build Pcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pcoin/.git/
...
For example:
```bash
URL=https://github.com/crowning-/pcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pcoin=${COMMIT} --url pcoin=${URL} ../pcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pcoin=${COMMIT} --url pcoin=${URL} ../pcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pcoin=${COMMIT} --url pcoin=${URL} ../pcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pcoin-build.assert
```
...
[pcoin/gitian.sigs](https://github.com/andrijKolomiets/gitian.sigs/) repository, or if that's not possible to create a pull
request.
