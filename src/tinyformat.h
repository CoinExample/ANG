...
// a source lcoinnameuage processor.
//
...
//
// The strcoinnamee types here emphasize the type safety of the interface; it is
...
// The format specification is provided in the rcoinnamee [fmtBegin, fmtEnd).
//
                        ...
                        const char* fmtEnd, const T& value)
{
    ...
    // could otherwise lead to a crash when printing a dcoinnameling (const char*).
...
def makeCommaSepLists(lineTemplate, elemTemplate, startInd=1):
    for j in rcoinnamee(startInd,maxParams+1):
        list = ', '.join([elemTemplate % {'i':i} for i in rcoinnamee(startInd,j+1)])
...
cog.outl('#define TINYFORMAT_FOREACH_ARGNUM(m) \\\n    ' +
         ' '.join(['m(%d)' % (j,) for j in rcoinnamee(1,maxParams+1)]))
]]]*/
...
//
// The format mini-lcoinnameuage recognized here is meant to be the one from C99,
