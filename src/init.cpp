...
static CCoinsViewDB *coinnamesdbview = NULL;
static CCoinsViewErrorCatcher *coinnamescatcher = NULL;
...
void PrepareShutdown()
{
    ...
    {
        LOCK(cs_main);
        if (coinnamesTip != NULL) {
            FlushStateToDisk();
        }
        delete coinnamesTip;
        coinnamesTip = NULL;
        delete coinnamescatcher;
        coinnamescatcher = NULL;
        delete coinnamesdbview;
        coinnamesdbview = NULL;
        delete pblocktree;
        pblocktree = NULL;
    }
#ifdef ENABLE_WALLET
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -blocknotify=<cmd>     " + _("Execute command when the best block chcoinnamees (%s in cmd is replaced by block hash)") + "\n";
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "coinname.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "coinnamed.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in COINNAME/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in COINNAME/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    strUsage += "  -rescan                  " + _("Rescan the block chain for missing wallet transactions") + " " + _("on startup") + "\n";
    strUsage += "  -salvagewallet           " + _("Attempt to recover private keys from a corrupt wallet.dat") + " " + _("on startup") + "\n";
    strUsage += "  -sendfreetransactions    " + strprintf(_("Send transactions as zero-fee transactions if possible (default: %u)"), 0) + "\n";
    strUsage += "  -spendzeroconfchcoinnamee     " + strprintf(_("Spend unconfirmed chcoinnamee when sending transactions (default: %u)"), 1) + "\n";
    ...
    strUsage += "  -walletnotify=<cmd>      " + _("Execute command when a wallet transaction chcoinnamees (%s in cmd is replaced by TxID)") + "\n";
    ...
    strUsage += "                           coinname (or specifically: darksend, instantx, masternode, keepass, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in COINNAME/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all Coinname specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0) + "\n";
    ...
    strUsage += "  -budgetvotemode=<mode>     " + _("Chcoinnamee automatic finalized budget voting behavior. mode=auto: Vote for only exact finalized budget match to my generated budget. (string, default: auto)") + "\n";
    ...
    strUsage += "  -anonymizecoinnameamount=<n>     " + strprintf(_("Keep N COINNAME anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Coinname Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Coinname is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    bSpendZeroConfChcoinnamee = GetArg("-spendzeroconfchcoinnamee", true);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Coinname Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Coinname Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Coinname version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (mapArgs.count("-whitelist")) {
        BOOST_FOREACH(const std::string& net, mapMultiArgs["-whitelist"]) {
            ...
            CNode::AddWhitelistedRcoinnamee(subnet);
        }
    }
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                coinnamesdbview = new CCoinsViewDB(nCoinDBCache, false, fReindex);
                coinnamescatcher = new CCoinsViewErrorCatcher(coinnamesdbview);
                coinnamesTip = new CCoinsViewCache(coinnamescatcher);
                ...
                // Check for chcoinnameed -txindex state
                if (fTxIndex != GetBoolArg("-txindex", true)) {
                    strLoadError = _("You need to rebuild the database using -reindex to chcoinnamee -txindex");
                    break;
                }

                uiInterface.InitMessage(_("Verifying blocks..."));
                if (!CVerifyDB().VerifyDB(coinnamesdbview, GetArg("-checklevel", 3),
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Coinname Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Coinname Core to complete") << "\n";
    ...
    nAnonymizeDarkcoinAmount = GetArg("-anonymizecoinnameamount", 0);
    ...
    LogPrintf("Anonymize Coinname Amount %d\n", nAnonymizeDarkcoinAmount);
