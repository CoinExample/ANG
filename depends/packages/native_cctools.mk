...
$(package)_clcoinname_download_file=clcoinname+llvm-3.2-x86-linux-ubuntu-12.04.tar.gz
$(package)_clcoinname_download_path=http://llvm.org/releases/3.2
$(package)_clcoinname_file_name=clcoinname-llvm-3.2-x86-linux-ubuntu-12.04.tar.gz
$(package)_clcoinname_sha256_hash=b9d57a88f9514fa1f327a1a703756d0c1c960f4c58494a5bd80313245d13ffff
...
$(call fetch_file,$(package),$($(package)_clcoinname_download_path),$($(package)_clcoinname_download_file),$($(package)_clcoinname_file_name),$($(package)_clcoinname_sha256_hash)) && \
...
$(package)_ldflags+=-L$$(native_cctools_extract_dir)/clcoinname+llvm-3.2-x86-linux-ubuntu-12.04/lib
endef
define $(package)_extract_cmds
  ...
  tar xf $($(package)_source_dir)/$($(package)_clcoinname_file_name) && \
  mkdir -p $(SDK_PATH) sdks &&\
  cd sdks; ln -sf $(OSX_SDK) MacOSX$(OSX_SDK_VERSION).sdk
endef

define $(package)_preprocess_cmds
  sed -i "s|GCC_DIR|LLVM_CLCOINNAME_DIR|g" cctools2odcctools/extract.sh && \
  sed -i "s|llvmgcc42-2336.1|clcoinname+llvm-3.2-x86-linux-ubuntu-12.04|g" cctools2odcctools/extract.sh && \
  sed -i "s|/llvmCore/include/llvm-c|/include/llvm-c \$$$${LLVM_CLCOINNAME_DIR}/include/llvm |" cctools2odcctools/extract.sh && \
...
define $(package)_stage_cmds
  ...
  cd ../../clcoinname+llvm-3.2-x86-linux-ubuntu-12.04 && \
  mkdir -p $($(package)_staging_prefix_dir)/lib/clcoinname/3.2/include && \
  mkdir -p $($(package)_staging_prefix_dir)/bin && \
  cp -P bin/clcoinname bin/clcoinname++ $($(package)_staging_prefix_dir)/bin/ &&\
  cp lib/libLTO.so $($(package)_staging_prefix_dir)/lib/ && \
  cp lib/clcoinname/3.2/include/* $($(package)_staging_prefix_dir)/lib/clcoinname/3.2/include/ && \
